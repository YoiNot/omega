...
import {
  ...
  registerSimSpine,
  makeAudioModel,
  setupDeltaReplication,
  incrementalRoundTrip,
  recoverCore,
  audioGraph,
  spatialParams,
...
export interface Demo {
  ...
  recorder?: Recorder;
  /** Deterministic GOAP AI system + full §14 AI stack (agents plan + navigate). */
  goap: AiStackSystem;
  /** Simulation spine (env/fire/eco/econ/trade) registered on coreWorld. */
  simSpine: SimSpine;
  /** Spatial audio model for the demo emitters. */
  audioModel: ReturnType<typeof makeAudioModel>;
  /** Multiplayer base: 2nd client reconciled from the server via net-delta. */
  deltaRepl: DeltaReplication;
  /** The procgen terrain the scenario + sim spine ride (for inspection/tests). */
  geoTerrain: ReturnType<typeof buildGeologyTerrain>;
  /** Navigation grid derived from the seeded terrain biomes. */
  navGrid: BooleanGrid;
  fixedDt: number;
  terrainSeed: string;
  step(): void;
  /** Push the current server state to the delta client + step audio one tick. */
  stepNetworkAndAudio(): void;
  /** True when the delta client has converged to the server's logical snapshot. */
  deltaConverged(): boolean;
  /** Recompute + return the deterministic audio graph for the current world. */
  audioFor(listener: { x: number; y: number; z: number }): ReturnType<typeof audioGraph>;
  /** Recompute spatial params for the given sources + listener (deterministic). */
  spatialFor(listener: { x: number; y: number; z: number }, sources: Parameters<typeof spatialParams>[2]): ReturnType<typeof spatialParams>;
  /** Save→reload round-trip proof on the current core world (deterministic). */
  saveLoadRoundTrip(createdAt: number): { equal: boolean; bytesLength: number };
  /** Recover a plain save of the current core world (corruption-path proof). */
  recoverCurrentSave(): { ok: boolean };
  /** Observable AI-stack view per agent (traits/goal/memory/relationships). */
...
export function createDemo(opts: DemoOptions): Demo {
  ...
  //
  // COUPLING (brief §4): the scenario's world is GENERATED by the real procgen
  // pipeline in @omega/geology (plate tectonics → hydraulic erosion → climate
  // via @omega/sim-env → biome classification) instead of the static
  // TerrainGenerator test terrain. buildGeologyTerrain returns a world-gen
  // Terrain, so nav + scenario + biome rules ride the procgen world unchanged.
  const geoTerrain = buildGeologyTerrain(`${seed}:geology`, terrainSize);
  ...
  // --- ai-stack: FULL §14 agent capability stack (Goap + Goals + Memory +
  //     Personality + Relationships + Learning) on the demo agents. Extends
  //     GoapSystem, so every existing gameplay/test surface (positions, plans,
  //     determinism oracle) keeps working unchanged. ---
  const goap = new AiStackSystem(coreWorld, liveGrid, seed);
  ...
  // --- sim spine: register weather/fire/eco/econ/trade onto the SAME
  //     engine-core world the demo steps. Because coreWorld.step(dt) runs every
  //     registered system, these now tick inside the time-core scheduler and
  //     couple (env → fire → eco → econ → trade) through the entity-0 singletons
  //     they each own. The env field is built from the procgen terrain, so the
  //     climate literally rides the world the agents walk on. ---
  const simSpine: SimSpine = registerSimSpine(coreWorld, seed, geoTerrain);

  // --- multiplayer base (net-delta): a 2nd client reconciled from the server
  //     over the same codec via @omega/net-delta deltas. Pushed every tick in
  //     step(); converges bit-for-bit to the server logical snapshot. ---
  const deltaRepl: DeltaReplication = setupDeltaReplication(codec);

  // --- audio: spatial model + deterministic playback graph for the demo
  //     emitters (agent/player/net positions). Stepped every tick so the graph
  //     tracks the world; headless-safe (mock AudioContextLike). ---
  const audioModel = makeAudioModel();
  ...
  const demo: Demo = {
    ...
    step(): void {
      ...
      scheduler.step(fixedDt, () => {
        ...
        // (c2) push the authoritative server state to the delta client so the
        // multiplayer base converges (net-delta path) — deterministic, no clock.
        deltaRepl.push(codec.toLogical(netServer));
    ...
    // --- full-slice observable hooks (brief §3–§8) ----------------------------
    /** Push server state to the delta client AND step the audio graph one tick. */
    stepNetworkAndAudio(): void {
      const serverLogical = codec.toLogical(netServer);
      deltaRepl.push(serverLogical);
      // Build the deterministic playback graph from current emitter positions;
      // uses the headless mock context so no real audio device is required.
      this.audioFor({ x: half, y: 2, z: half });
    },
    /** True once the delta client serializes identically to the server. */
    deltaConverged(): boolean {
      return deltaRepl.converged(codec.toLogical(netServer));
    },
    /** Deterministic audio graph for the current world (inject a ctx to capture). */
    audioFor(
      listener: { x: number; y: number; z: number },
      ctx: AudioContextLike = makeMockAudioContext(),
    ): ReturnType<typeof audioGraph> {
      const sources = this.audioSources();
      return audioGraph(ctx, audioModel, { pos: new Vec3(listener.x, listener.y, listener.z), forward: new Vec3(0, 0, -1) }, sources, {
        masterGain: 1,
        frequencies: Object.fromEntries(sources.map((s) => [s.id, 220 + (s.id.charCodeAt(s.id.length - 1) % 6) * 40])),
      });
    },
    /** Deterministic spatial parameters for explicit sources + listener. */
    spatialFor(listener: { x: number; y: number; z: number }, sources: AudioSourceInput[]): SpatialSourceParam[] {
      return spatialParams(audioModel, { pos: new Vec3(listener.x, listener.y, listener.z), forward: new Vec3(0, 0, -1) }, sources);
    },
    /** Gather the demo's emitters as audio sources (agents + player + net). */
    audioSources(): AudioSourceInput[] {
      const out: AudioSourceInput[] = [];
      for (const a of goap.positions()) {
        out.push({ id: `agent-${a.id}`, pos: new Vec3(a.tx + 0.5, 1, a.tz + 0.5), gain: 1 });
      }
      for (const p of playerSys.players()) {
        out.push({ id: `player-${p.id}`, pos: new Vec3(p.tx + 0.5, 1, p.tz + 0.5), gain: 1 });
      }
      for (const n of this.netPositionsServer()) {
        out.push({ id: `net-${n.id}`, pos: new Vec3(n.x, n.y, n.z), gain: 1 });
      }
      return out;
    },
    /** Save→reload round-trip proof on the current core world. */
    saveLoadRoundTrip(createdAt: number): { equal: boolean; bytesLength: number } {
      const rt = incrementalRoundTrip(coreWorld, seed, createdAt);
      return { equal: rt.equal, bytesLength: rt.bytes.length };
    },
    /** Recover a plain save of the current core world (corruption-path proof). */
    recoverCurrentSave(): { ok: boolean } {
      return recoverCore(coreWorld);
    },
    /** Observable AI-stack view per agent (traits / goal / memory / practice). */
    aiStackViews() {
      return goap.stackViews();
    },
    /** Serializable social network of the demo agents. */
